...
CONTROL = enum(NORMAL = 11, WARNING = 12, UPDATE = 13, HELLO = 14, ERROR = 15, RESULT = 16)
MESSAGE_TYPE = enum(UPDATE = 21, HELLO = 22, ACK = 23, DATA = 24, DATAACK = 25, SERVER = 26, SERVERACK = 27)
CONTROLLER = enum(PULL = 31, PUSH = 32, UPDATE = 33, SERVERACK = 34, DATAACK = 35)
...
def main(screen):
    ...
    global TOTAL_NODES, SERVER, GATEWAY, SERVER_CONVERGENCE, NTP_ADDRESS, CONTROLLER_ADDRESS
    ...
    print_screen(screen, CONTROL.NORMAL, "RMP controller initialising ... (RMP pre-alpha, version 0.6)")
    # start threads now
...
def handle_communication(screen, start):
    ...
    while True:
        ...
        try:
            ...
            # if I am the server, send Data ACK back to source node, send DATA to controller (laptop)
            if (msg_type == MESSAGE_TYPE.DATA):
                ...
                if (SERVER == HOST):  # if I am the server, recorde the timestamp, send to laptop, send ACK back to source node
                    ...
                    send_sensor_event_to_controller(controller_address, timestamp)
...
# SERVER -> CONTROLLER
def send_sensor_event_to_controller(controller_address, timestamp):
    message = str(CONTROLLER.UPDATE)
    message = message + ' ' + str(SERVER) + ' ' + str(timestamp)

    # Send UDP datagram
    socket_s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    socket_s.settimeout(UPDATE_TIMEOUT)
    socket_s.sendto(message, (controller_address, PORT))
